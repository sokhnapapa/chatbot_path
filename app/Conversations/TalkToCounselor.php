...
class TalkToCounselor extends Conversation
{
    ...
    public function sendContact()
    {
        ...
        $question = Question::create('You can choose one of these actions')
            ...
            ->addButtons([
                ...
                Button::create('Request CallBack')->value('rqst_clbck'),
            ...
            if ($answer->isInteractiveMessageReply()) {
                ...
                if ($selectedValue == 'ask_qn') {
                    $this->ask('Please go ahead and ask your question.', function (Answer $answer) {
                        ...
                        $qn->body = $this->quest;
                        $qn->save();
                        $this->sendConversationRapidpro($user->getId(), "Question: " . $answer->getText());
                        $this->say('Your question has been received. You will get a reply soon');
                    });
                } elseif ($selectedValue == 'rqst_clbck') {
                    $this->ask('Please type your phone number in order to be called.', function (Answer $answer) {
                        $this->quest = $answer->getText();
                        $qn = new \App\Question;
                        $user = $this->bot->getUser();
                        $qn->psid = $user->getId();
                        $qn->body = $this->quest;
                        $qn->save();
                        $this->sendConversationRapidpro($user->getId(), "CallBackRequest: " . $answer->getText());
                        $this->say('Call back request has been received. You will be called back by one of our agents.');
                    });
    ...
    public function sendConversationRapidpro($userPsid, $message)
    {
        ...
        file_get_contents($serverDetails->Url, false, $context);
